...
from telegram.ext import (run_async, Updater, Filters, CommandHandler,
...
class TelegramBOT():
  ...
  def start(self, update: Update, context: CallbackContext) -> int:
    ...
    update.effective_chat.send_message(
      "💫 Welcome To Youtube Video Converter 💫",
  ...
  def end(self,update: Update, context: CallbackContext) -> int:
        ...
        text="Runforever set to: {}!".format(Runforever[0])
        update.effective_chat.send_message(text,
      parse_mode=ParseMode.HTML,
    )

  def usercount(self, update: Update, context: CallbackContext) -> int:
    UserCounter = 0
    for file in os.listdir("Data/"):
      UserCounter += 1
    update.effective_chat.send_message(
      f"Currently {UserCounter} user are using your bot",
      parse_mode=ParseMode.HTML,
    )
    ...
    if update.message == None:
      ...
      if query.answer():
        ...
        res = update.effective_chat.send_message(
          f"Converting {query.message.reply_markup.inline_keyboard[0][0]['text']} \nPlease wait... ",
          parse_mode=ParseMode.HTML,
          timeout=6000)
        self.SentMessages[str(update.callback_query.from_user.id)].append(
          res.message_id)
        Mp3 = self.download_link(str(update.callback_query.from_user.id),
                                 query.data)
        ...
        for line in self.SentMessages[str(update.callback_query.from_user.id)]:
          time.sleep(1)
          try:
            context.bot.delete_message(update.callback_query.from_user.id,
                                       line, 60)
      ...
      try:
        try:
          res = update.effective_chat.send_message(
            "`{} 🤔🤔 ` \n 🔎 Looking for your video 🔍 \.\.\. ".format(
              update.message.text),
            parse_mode=ParseMode.MARKDOWN_V2,
          )

          self.SentMessages[str(update.message.from_user.id)].append(
            res.message_id)
        except:
        ...
        if self._YoutubeData(Message, update.message.from_user.id) == False:
          ...
          Data = self._ReadMusicDetails(str(update.message.from_user.id),
                                        "Image")
          if Data[0] == {}:
            res = update.effective_chat.send_message(
          ...
          for myimage in Data:
            Mydata = self._ReadMusicDetails(str(update.message.from_user.id),
                                            "Image",
                                            Conditional=True,
                                            Value=myimage)
            ##print(Mydata)
            keyboard = [
              [
                InlineKeyboardButton(myimage[:-4],
                                     callback_data=Mydata["Video_Url"])
              ],
            ]
            ...
            res = update.effective_message.chat.send_photo(
              photo=open(f"Data/{str(update.message.from_user.id)}/{myimage}",
                         'rb'),
            ...
            self.SentMessages[str(update.message.from_user.id)].append(
              res.message_id)
          res = update.effective_chat.send_message(
            "Please \n`Select Your Video By clicking on the Video's Title below the images`",
            parse_mode=ParseMode.MARKDOWN_V2,
          )
          self.SentMessages[str(update.message.from_user.id)].append(
            res.message_id)
  ...
  def downloadImages(self, img_url, filename, Api_ID=None):
    try:
      ...
      if response.status_code:
        ...
        color_clip = ColorClip(size=(int(im_width * 1.1),
                                     int(im_height * 1.4)),
                               color=(0, 25, 51))
      ...
      #print("This is my error " + e.__str__())
      ...
      for video in s.results:
        ...
        if counters <= 5:
          try:
            ...
            if self.downloadImages(video.thumbnail_url, filename,
                                   Api_ID) == True:
              MyMusics.append({
                "Title": filename,
                "Image": filename + ".png",
  ...
  def move_and_remove(Api_ID=None, Format=None):
    ...
    if Format == None:
      ...
      for file in listdir(path):
        if file.endswith(".mkv") or file.endswith(".mp4") or file.endswith(
            ".mp3") or file.endswith(".png"):
  ...
  async def main() -> None:
    try:
      ...
      dispatcher.add_handler(
        CommandHandler("start", mytelegram.start, run_async=True))
      dispatcher.add_handler(
        CommandHandler("usercount", mytelegram.usercount, run_async=True))
      dispatcher.add_handler(
        CommandHandler("restart", mytelegram.restart, run_async=True))
      dispatcher.add_handler(
        CommandHandler('stop', mytelegram.end, run_async=True))
      dispatcher.add_handler(
        CommandHandler("about", mytelegram.about, run_async=True))
      dispatcher.add_handler(
        CommandHandler('cancel', mytelegram.cancel, run_async=True))
      dispatcher.add_handler(
        CommandHandler("search", mytelegram.search, run_async=True))
      dispatcher.add_handler(
        CommandHandler('help', mytelegram.help, run_async=True))
      dispatcher.add_handler(
        MessageHandler(Filters.all, mytelegram.AllMessages, run_async=True))
      dispatcher.add_handler(
        CallbackQueryHandler(mytelegram.AllMessages, run_async=True))
      # Start the Bot
